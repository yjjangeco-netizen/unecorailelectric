-- 일괄 입고 트랜잭션 처리 함수
-- 에러 발생 시 완전 롤백 보장
CREATE OR REPLACE FUNCTION process_bulk_stock_in_transaction(
  p_id UUID,
  p_product TEXT, 
  p_spec TEXT, 
  p_maker TEXT, 
...
DECLARE 
  v_item_id UUID;
  v_location TEXT := '창고A'; -- 기본 위치 설정
BEGIN
  -- 입력값 검증
  IF p_id IS NULL THEN
    RAISE EXCEPTION '품목 ID는 필수입니다';
  END IF;
  ...
  -- 디버깅용 로그
  RAISE NOTICE '일괄 입고 처리 시작 - ID: %, 품목: %, 규격: %, 상태: %, 수량: %', p_id, p_product, p_spec, p_stock_status, p_quantity;
  
  -- 1단계: items 테이블에 품목 정보 삽입 (지정된 ID 사용)
  INSERT INTO items (
    id, product, spec, maker, unit_price, purpose, note, stock_status, created_at, updated_at
  ) VALUES (
    p_id, p_product, p_spec, p_maker, p_unit_price, p_purpose, p_note, p_stock_status, NOW(), NOW()
  ...
  -- 성공 로그
  RAISE NOTICE '일괄 입고 처리 완료 - 품목 ID: %, 상태: %', v_item_id, p_stock_status;
...
EXCEPTION 
  WHEN OTHERS THEN
    -- 모든 에러를 로그하고 롤백
    RAISE NOTICE '일괄 입고 처리 중 오류 발생: %', SQLERRM;
    RAISE EXCEPTION '일괄 입고 처리 중 오류: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
