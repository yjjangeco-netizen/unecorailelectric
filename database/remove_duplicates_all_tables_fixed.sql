...
-- 5단계: 외래키 무결성 확인 및 정리
-- stock_history.item_id가 items.id에 없는 경우 제거
DELETE FROM stock_history
WHERE NOT EXISTS (
...
-- 9단계: 외래키 무결성 최종 확인
SELECT '=== 외래키 무결성 최종 확인 ===' as info;
SELECT
  'stock_history.item_id가 items.id에 없는 경우' as check_type,
  COUNT(*) as count
FROM stock_history sh
WHERE NOT EXISTS (
  SELECT 1 FROM items i WHERE i.id = sh.item_id
);
